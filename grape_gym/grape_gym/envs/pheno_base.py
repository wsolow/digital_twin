...
from grape_model import NASAPowerWeatherDataProvider


class NPK_Env(gym.Env):
    """Base Gym Environment for simulating crop growth
    
    Relies on the Grape_Model package (in base folder) and the GrapePhenologyModel crop model. 
    """
    ...
    def __init__(self, args: NPK_Args, base_fpath: str, agro_fpath:str, \
        ...
        crop = gp.fileinput.YAMLCropDataProvider(fpath=os.path.join(base_fpath, crop_fpath))
        site = gp.fileinput.YAMLSiteDataProvider(fpath=os.path.join(base_fpath, site_fpath))

        self.parameterprovider = gp.base.ParameterProvider(sitedata=site, cropdata=crop)
    ...
    def step(self, action):
        ...
        self._log(output.iloc[-1]['DVS'], act_tuple, reward)
    ...
    def _validate(self):
        ...
        # Check that DVS is present
        if 'DVS' not in self.output_vars:
            msg = 'Crop State \'DVS\' variable must be in output variables'
            raise exc.WOFOSTGymError(msg)
    ...
    def _get_reward(self, output: dict, act_tuple: tuple):
        ...
        """
        return np.nan_to_num(output.iloc[-1]['DVS'])
