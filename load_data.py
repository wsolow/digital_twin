...
RENAME_COLUMN_MAP = {"SR_MJM2":"IRRAD", "P_INCHES":"RAIN", "MEAN_AT":"TEMP", "MAX_AT":"TMAX", "MIN_AT":"TMIN",}
...
def load_and_process_data_phenology(cultivar: str):
    """
    Load and process AgAid GitHub repository data
    ...
    df.drop(columns=["AWN_STATION", "SEASON", "SEASON_JDAY", "LTE10", "LTE50", "LTE90", 
              "PREDICTED_LTE10", "PREDICTED_LTE50", "PREDICTED_LTE90", 
             "PREDICTED_BUDBREAK", "MIN_ST2", "ST2", "MAX_ST2", "MIN_ST8", "ST8", "MAX_ST8",
             "SM8_PCNT", "SWP8_KPA", "MSLP_HPA", "LW_UNITY", "ETR", "ETO"],inplace=True) #Removed "ETO" from list
    ...
    df.drop(columns=["AVG_AT", "WD_DEGREE", "MIN_RH", "AVG_RH", "MAX_RH", "MIN_DEWPT", "AVG_DEWPT", "MAX_DEWPT", "WS_MPH", "MAX_WS_MPH"], inplace=True)
    # Removed "MIN_RH", "AVG_RH", "MAX_RH", "MIN_DEWPT", "AVG_DEWPT", "MAX_DEWPT", "WS_MPH", "MAX_WS_MPH"
    ...
    if len(df_list) != 1:
        ...
        for i,df in enumerate(df_list): np_arr_df[i] = df
        for i,s in enumerate(stages_list): np_arr_stages[i] = s
        return np_arr_df, np_arr_stages

def load_and_process_data_phenology_reduced(cultivar: str):
    """
    Load and process AgAid GitHub repository data
    Do not assume dormancy data is available
    """
    ...
    df.drop(columns=["AWN_STATION", "SEASON", "SEASON_JDAY", "LTE10", "LTE50", "LTE90", 
              "PREDICTED_LTE10", "PREDICTED_LTE50", "PREDICTED_LTE90", 
             "PREDICTED_BUDBREAK", "MIN_ST2", "ST2", "MAX_ST2", "MIN_ST8", "ST8", "MAX_ST8",
             "SM8_PCNT", "SWP8_KPA", "MSLP_HPA", "LW_UNITY", "ETR"],inplace=True) #Removed "ETO" from list
...
def main():
    ...
    load_and_process_data_phenology_reduced(args.cultivar)
